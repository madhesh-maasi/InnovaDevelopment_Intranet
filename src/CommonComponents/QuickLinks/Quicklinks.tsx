...
const Quicklinks: React.FC<IQuickLink> = (props:any) => { 
  const tenantUrl = useSelector((state: any) => state?.MainSPContext?.tenantUrl);
  const imgUrl = props.Logo ? `${tenantUrl}${props.Logo}` : "";
  return (
    <a href={props.Link} target="_blank" rel="noopener noreferrer" className={styles.quickLinkcard}>
